...
int phaseR = 0, phaseG = 0, phaseB = 0; // 0: stationary, 1 or more: moving
float speed = 1.5; // packet movement speed
String currentPacket = ""; // currently moving packet (used in Circuit mode)
boolean isCircuitMode = true; // initial mode is Circuit
...
void draw() {
  ...
  if (isCircuitMode && !currentPacket.equals("")) {
    fill(255, 0, 0);
    textSize(25);
    textAlign(CENTER, CENTER);
  ...
  if (currentPacket.equals("R") || !isCircuitMode) moveRed();
  if (currentPacket.equals("G") || !isCircuitMode) moveGreen();
  if (currentPacket.equals("B") || !isCircuitMode) moveBlue();
}

void drawPacket(float x, float y, color c, String label) {
  fill(c);
  rect(x, y, 25, 25);
  fill(0);
  textSize(15);
